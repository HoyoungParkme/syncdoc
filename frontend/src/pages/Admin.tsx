...
export function Admin() {
  ...
  const [confirmKind, setConfirmKind] = useState<'rebuild' | 'restore' | 'delete'>('rebuild')
            ...
            {repos.map((r, i) => (
              <tr key={r.code} data-el={i === 0 ? '2.1' : undefined}>
                ...
                <td>
                  ...
                  {/* 위험색 외곽선 — 토큰 폐기(UI-13 3.2)와 같은 어휘. 「해제」 두 글자라야 셋이 한 칸에 가로로 선다 */}
        ...
        {confirm && (
          <>
            ...
            <div className="dialog" data-el="4" style={{ '--depth': 1 } as React.CSSProperties}>
              <div className="dhead">
                {confirm} {confirmKind === 'rebuild' ? '인덱스 재구축' : confirmKind === 'restore' ? '백업에서 복원' : '싱크독에서 해제'}
              </div>
              <div className="dbody">
                {confirmKind === 'rebuild' ? (
                  <>
                  ...
                  <>
                    ...
                    {/* 4.3 — 되돌릴 수 있는 것과 없는 것. 백업 유무는 그 행의 값을 그대로 말한다 */}
                    <div className="banner warn" data-el="4.3">
                      <b>GitHub 저장소는 그대로 남습니다</b> — 명세 원본은 거기 있습니다. 다시 등록하면 문서가 git에서 복원됩니다. 플래그·전파 결정·댓글은{' '}
                      <b>backup/tracking.json</b>이 있을 때만 돌아옵니다 —{' '}
                      {repos.find((x) => x.code === confirm)?.backed_up_at ? '이 프로젝트는 백업이 있습니다' : '이 프로젝트는 백업이 없습니다'}
                    </div>
                  </>
                )}
                <div className="dacts">
                  ...
                  <button
                    ...
                    disabled={busy}
                    onClick={() => (confirmKind === 'rebuild' ? rebuild(confirm) : confirmKind === 'restore' ? restore(confirm) : remove(confirm))}
                  >
                    {busy ? '도는 중…' : confirmKind === 'rebuild' ? '재구축' : confirmKind === 'restore' ? '복원' : '해제'}
                  </button>
                </div>
              </div>
            </div>
