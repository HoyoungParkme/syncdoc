...
| 화면 | UI-16에 절 하나 — 「에이전트를 붙이는 법」 표(6) + `claude mcp add` 명령 상자(6.2, 주소 채움) + 그 다음(6.3) |
| 테스트 | `check_ui.py` UI-16 요소 14/14 · 명령 상자의 주소가 지금 화면의 origin과 같다 |
| 선행 | F |
...
**왜 지금인가.** 붙이는 절차를 설명서로 앱 밖(claude.ai 아티팩트)에 만들었더니, 다른 컴퓨터·다른 프로젝트에서 붙이려던 사람이 그걸 못 보고 "syncdoc 명령이 없다"에서 멈췄다. 가장 자주 걸리는 한 줄 — **MCP 서버는 세션 시작 때 읽히니 새로 켜야 한다** — 이 앱 안에 있어야 한다. 캡처는 넣지 않는다. 그 화면들이 한 클릭 거리다.
