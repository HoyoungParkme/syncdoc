...
**진행 상황**: 카드 44장. **A~AJ 44장 완료**(2026-09-23).
...
#### AJ 단계 행이 무엇인지 말하고, 다이얼로그는 Esc로 닫힌다

| 항목 | 내용 |
|---|---|
| 근거 | [[SYNC-UI-002]] 1.1 · [[SYNC-UI-002#UI-4]] 4.1·4.4·4.5 · #116 · #117 · 사용자 결정 2026-09-23 |
| 구현 | `ui.tsx` `useEscape` — 모듈 스택 하나, 맨 위 하나만, 한글 조합 중엔 무시. 다이얼로그 열 곳과 그림 전체보기가 같이 쓴다(재구축 중엔 막힘). 배경이 빠져 있던 셋(UI-7 삭제 확인 · UI-5 휴지통 확인 · UI-3 기존 명세 발견)에 배경. UI-4 단계 행·표준 행: 이름은 UI-16과 같은 사람 말 + 작은 코드(`STAGE_NAMES` 하나를 둘이 같이), `문서 N개 ▸`, 접힌 행에 문서 링크 전부(4.5, 넘치면 줄바꿈, 펼치면 사라짐), 툴팁 「눌러서 문서 펼치기/접기」(이름·오른쪽만) |
| 테스트 | `check_ui` 12/12(UI-4에 4.5) · 사람이 배포 뒤 UI-4를 `data-el`대로 누르고, 다이얼로그마다 Esc·바깥 좌표 클릭·겹친 다이얼로그·한글 조합 중 Esc |
| 선행 | — |
| 완료 | 2026-09-23 · 브랜치 `card/AJ-handles` · spec 2 + code 2 + 정렬 후속 1(#162) · `check_ui` **12/12**(UI-4에 4.5) · `check_tokens` 91/0 · `validate` 0/0 · `check_code` 114/114 · `check_dom` · `check_view_css` · `check_templates` · `open_items --check` · `view_build --selftest` · 테스트 242 · 사람 확인(배포 뒤 브라우저, `data-el`대로): UI-4 — 이름·작은 코드·`문서 N개 ▸`, 접힌 행에 ID(SYNC MINISPEC 7개는 둘씩 네 줄), ID를 누르면 UI-5로 가고 행은 안 접힘, 이름을 누르면 펼쳐지고 ID 줄이 사라짐, 툴팁은 이름·오른쪽에서만(ID 위 없음) 「눌러서 문서 펼치기」→펼친 뒤 「눌러서 접기」 · Esc — 사용 방법 · 설정 · UI-14 재구축 확인이 설정 위에 겹쳤을 때 한 번에 위만·두 번에 둘 다 · UI-5 휴지통 확인(바깥 좌표 클릭도, 한글 조합 중 Esc는 무시) · UI-7 되돌리기 확인 · UI-3 초기화 · 그림 전체보기(아무것도 실행하지 않고 열고 닫기만) · UI-4 목록(6)·완전 삭제 확인(8.4)·기존 명세 발견(4)·UI-15는 띄울 조건이 없어 코드와 빌드로만 · **되먹임**: 툴팁 감싸개(`.tipwrap`, inline-block)가 이름 칸 폭과 캐럿 간격을 먹어 필이 들쭉날쭉했다 — #162로 단계 행 안에서만 inline-flex. 고친 문서가 초안일 때는 「상위 미완료」 배지가 자리를 차지해 MINISPEC ID가 한 줄에 하나(7줄)였다 |

**왜 카드인가.** 둘 다 명세를 바꾼다. #116은 코드 버그로는 재현되지 않았다 — 문서 행을 누르면 UI-5로 간다. 단계 이름이 코드뿐이라 문서 ID의 일부처럼 읽혀서 처음 온 사람이 단계 행을 문서로 알고 누른 것이다. 화면이 헷갈리게 생긴 것을 명세에서 고친다. #117의 Esc는 1.1에 없던 규칙이다. 「배경 클릭이 막힌다」는 자동화 도구가 `.backdrop` 한가운데를 눌러 다이얼로그에 가로채인 착시였다 — 사람이 카드 밖을 누르면 닫힌다.

---

#### AK 시나리오 뷰가 원본 문장을 버리지 않는다

| 항목 | 내용 |
|---|---|
| 근거 | [[SYNC-STD-002]] 1장·V-SCN · [[SYNC-STD-001]] 2.3 · #152 · 사용자 결정 2026-09-23 |
...
**왜 아무도 못 봤나.** 절을 두 번 그린 사본이 원본 그대로라, 카드에서 사라진 줄이 바로 위 사본에 다 있었다. SYNC 자기 시나리오는 S가 `###`이고 단계·변형이 한 줄씩이라 카드에서 잃는 것이 없었다.

---

#### AL 화면 뷰가 원본 문장을 버리지 않는다

| 항목 | 내용 |
|---|---|
| 근거 | [[SYNC-STD-002]] 1장·V-UI · [[SYNC-STD-001]] 2.7 · #152 · 사용자 결정 2026-09-23 |
| 구현 | `wf_build.py`·`wireframe.ts` — 화면 블록을 줄 단위로 가른다(코드 펜스 인식): 머리(두 칸 표는 메타, 나머지는 한 줄 목적) · 첫 html = 배치 · 요소 소절 첫 표(문서 머리·칸 그대로) · 규칙(`- ` + 이어진 줄) · 시나리오(`**S-N 제목**` 뒤 나머지는 유스케이스 자리, 단계 + 이어진 줄). 안 쓰인 줄은 화면 끝 「그 밖」(둘째 html은 공통 틀과 함께). 설계 화면은 표 대신 `item_card`/`itemCard` 카드. 앱의 묶음·화면 HTML을 정적과 같게(묶음 한 겹, 요소 표 머리, 한 줄 목적 블록, html 블록 한 겹 `wfbox`). `render_blocks`에 공통 틀 인자 · `etc_block`은 구분선뿐이면 그리지 않음 · 단계·규칙이 같이 쓰는 `lead_rest`. CSS: `.rsec` 사이 선을 위로, `.rsecs>.etc`, `.s-desc p` |
| 테스트 | `view_build --selftest`에 V-UI(설계 카드·몸 전부·하위 N · 메타·머리 설명 · 요소 표 문서 머리·칸 모자란 행 · 규칙 이어진 줄·밑 목록·칩 · 시나리오 ` . [[…]]` · 그 밖 순서·둘째 html 공통 틀 · 구분선뿐인 화면) · UI 문서 열(운영 아홉 + BBS)에서 사라진 조각 0 — 정적과 앱 둘 다 · 앱 포트를 일회성으로 묶어 화면 속 HTML이 정적과 같은지 · 사람이 배포 뒤 SYNC·VA·TBL·JSD·HB 화면 문서를 브라우저에서 |
| 선행 | AK |
| 완료 | 2026-09-23 · 브랜치 `card/AL-ui-view` · spec 4 + code 2 · 테스트 243 · `validate` 0/0 · `check_code` 114/114 · `check_ui` 12/12 · `check_dom` 10·10·10 · `check_tokens` 91/0 · `check_view_css` 네 쌍 · `check_templates` 16/16 · `open_items --check` · `wf_build --selftest` · **`view_build --selftest` V-UI 12건**(옛 `wf_build`로 바꿔 돌리면 10건 실패) · UI 문서 열하나(운영 아홉 + BBS 둘)에서 사라진 조각 0 — 정적·앱 둘 다. 전에는 VA-UI-001 182 · TBL 108 · INS 25 · SYNC-UI-001 13 · BBS 11 · JSD 11 · SYNC-UI-002 2 · 앱 포트를 일회성으로 묶어 열두 문서에서 설계 카드·탭·화면 조각이 정적 뷰와 **같은 HTML**(앱 전용 `data-item`·`data-ref`·프레임 버튼 `btn sm`·srcdoc 속 빼고) · 사람 확인(배포 뒤 2026-09-23, 브라우저): SYNC-UI-001 카드 12장·표 없음·요약 문단이 절 머리·`하위 2`·바닥 줄 · VA-UI-001 카드 7장(UI-1에 표·목록·규칙 전부) · TBL UI-10 그 밖에 배치 소절의 근거 패널 설명과 둘째 배치, 「예외 상태」 제목·표 둘·보드 — 시나리오 넷이 처음으로 보이고 유스케이스 자리에 링크. TBL은 공통 틀이 없고 보드의 클래스 스타일이 문서 어디에도 없어 보드는 모양 없이 보인다 · JSD UI-3 그 밖에 배치 소절 글 · SYNC-UI-002 UI-4 규칙 이어진 줄, UI-7 칸 모자란 행(앱이 버리던 것), 화면 끝 구분선은 그리지 않음, 표 행 클릭 → 배치 강조, 탭 전환, `#item-UI-4` 진입 · HB 네 화면 그대로 · **되먹임**(범위 밖 둘, 이슈로): #172 공통 틀 헤딩 바로 아래 html을 못 찾는다(자체 시험 문서를 쓰다 드러남) · #173 앱 배치 격리가 `<form>` 안 요소를 통째로 지우고 공통 틀 link·style 순서를 바꾼다(INS-UI-002 UI-6·BBS-UI-002 입력칸이 앱에만 없다) · 남은 것: #152 UC 카드 |

**왜 카드인가.** 규약(STD-002 V-UI)이 설계 화면을 「표 한 행」으로 정했다 — 첫 줄만 싣는 것이 규약대로였다(AK와 같은 기준).

**왜 아무도 못 봤나.** SYNC 자기 문서는 설계 화면이 한 문단씩이고 배치 화면이 템플릿 소절만 써서 잃는 것이 거의 없었다(규칙의 이어진 줄 두 줄). 다른 저장소가 목록·상태 표·예외 소절을 쓰자 드러났다.

---

#### AM 유스케이스 뷰가 원본 문장을 버리지 않는다

| 항목 | 내용 |
|---|---|
| 근거 | [[SYNC-STD-002]] V-UC·3장·4장 · [[SYNC-STD-001]] 2.4 · #152 · 사용자 결정 2026-09-23 |
| 구현 | `view_build.py`·`uc.ts` V-UC 다시 — 원본 순서로 펼친다(`split_sections`·`split_items`, 절·소절 머리 그대로). 유스케이스는 `item_card`/`itemCard` 카드, 몸은 `uc_parts`/`ucParts`(줄 단위, 코드 펜스 인식)로 — 항목 표(원문 행 그대로, 긴 이름·짧은 이름 둘 다 관계로), 기본 흐름(여럿, 이어진 줄은 `lead_rest`), 확장(머리 뒤 글·들여 쓴 줄까지), 사후조건 참고·연관, 나머지는 「그 밖」. 맨몸 `UC-xx` 링크. 대응표는 원본 표 + 「유스케이스」 열 칩. 패키지 그림은 뷰 끝(액터 밑 싱크독 이름 삭제), 정적은 `UC_JS`. `UC_CSS`↔`ucCss`를 `check_view_css` 다섯째 쌍. 옛 `tools/parse.py`·`build.py`·`data.json` 삭제 |
| 테스트 | `view_build --selftest`에 V-UC(시험 문서 — 0장·소절 머리·미결 제자리 · 원문 표·모르는 행·규약 이름 관계 · 흐름 둘·단계 둘째 줄 · 확장 머리 뒤 글·`1a1.` · 사후조건 둘째 줄 · 그 밖 · 표 없는 항목 · 맨몸 링크 · 칩은 유스케이스 열만 · 옛 SYNC 데이터 안 나옴) · UC 문서 열하나에서 사라진 조각 0 — 정적·앱 둘 다 · 앱 포트를 일회성으로 묶어 본문이 정적과 같은 HTML · 사람이 배포 뒤 SYNC·KB·INS·BBS·TBL·IMBC를 브라우저에서 |
| 선행 | AL |
| 완료 | 2026-09-23 · 브랜치 `card/AM-uc-view` · spec 5 + code 2 · 테스트 243 · `validate` 0/0 · `check_code` 114/114 · `check_ui` 12/12 · `check_dom` 10·10·10 · `check_tokens` 91/0 · **`check_view_css` 다섯 쌍**(유스케이스 뷰 CSS 신설 — 뷰 CSS 상수 정규식이 `UC_CSS`를 잘못 잡아 줄 머리로 고침) · `check_templates` 16/16 · `open_items --check` · `wf_build --selftest` · **`view_build --selftest` V-UC 15건** · 옛 `v_uc`는 새로 클론한 곳에서 `data.json`이 없어 `FileNotFoundError`로 돌지도 못했다(시험 문서로 확인) · UC 문서 열하나(운영 열 — 컨테이너에서 읽기만 — + BBS)에서 사라진 조각 0 — 정적·앱 둘 다(전에는 앱이 0장·절 머리·흐름 변형·확장 이어진 줄·비 Cockburn 항목을 버렸다) · 앱 포트를 일회성으로 묶어 열두 문서에서 본문(그림 앞)이 정적 뷰와 **같은 HTML**(앱 전용 `data-item`·`data-ref` 빼고) · 정적 뷰를 브라우저로(잠깐 띄운 http.server): SYNC 카드 27·패키지 탭 셋·`#item-UC-H3` → 그 탭·타원 → 카드·H18 흐름 셋 · 사람 확인(배포 뒤 2026-09-23, 브라우저): SYNC 0장~4장·패키지 그림, 카드 27(하위 줄 26), 칩 75, 표 칸 `<br>` 줄바꿈 16 · INS 카드 18이 본문 그대로(「그 밖」 머리 없이) · TBL·IMBC 대응표 칸 전부(`[[#UC-…]]`은 링크), IMBC 5장 미결이 처음으로 보임 · QBOT 칩 69(`UC-A1` 모양) · 타원 클릭 → 카드, 카드 클릭 → 참조 패널, 칩 → 그 카드 · KB는 다른 사람 프로젝트라 브라우저로는 못 보고 파일로만(규약 이름 `확장점` → 확장 선) · **되먹임**: 대응표의 `[[#UC-H1]]` 링크는 칩이 아니라 참조 링크 모양이다(규칙대로 — 칩은 맨몸 ID만) · 추상 유스케이스(SYNC UC-A0)는 패키지 그림에서 액터 선이 아래 추상 타원으로 이어진다(옛 그림도 같았다) |
...
| (#169) | `fix/169-steps-scope` | — | #169 | 2026-09-23 |
