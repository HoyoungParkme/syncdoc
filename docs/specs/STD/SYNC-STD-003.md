...
**SYNC-DOM-003**
- [ ] `versions.body`가 커지면 압축할지 — 지금은 안 함
- [ ] DB 유실 시 복구 불가 항목(플래그·전파결정·댓글)의 백업 — 인프라 미결사항과 같음

**SYNC-UI-001**
- [ ] 저장소 동기화 상태를 UI-4에 노출할지
- [ ] 모바일 대응 여부 — 현재 데스크톱만 전제
...
**SYNC-STD-001**
- [ ] 아직 못 정한 것
- [ ] 절 헤딩 번호 누락을 위반으로 올릴지 — 지금은 권장. 단어형 패턴 타입(DOM·MS·MCP)에서만 위반으로 하는 절충안 가능
- [ ] DOM 세 문서의 이름 일치를 시스템이 검사할지 (도메인 `Document` ↔ 클래스 `Document` ↔ 테이블 `documents`)
