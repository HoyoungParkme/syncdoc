...
- **색·글꼴·간격은 [[SYNC-UI-001]] 3장이 원본이고 CSS 변수가 그 전사다.** 값을 컴포넌트에 직접 쓰지 않는다 — 같은 회색이 화면마다 조금씩 달라진다. 토큰에 없는 값이 필요하면 명세를 먼저 고친다
- 검사기 `tools/check_tokens.py` — 3장 표의 값 집합과 `:root`의 CSS 변수 값 집합을 대조. 이름은 안 본다(명세는 한국어 용도, CSS는 영어 이름이라 대조표가 하나 더 생기면 그것이 또 갈라진다). 타이포만 `14.5~15px` 같은 폭을 허용한다
- **여러 화면이 같이 쓰는 조각은 UI-002 1장 공통 컴포넌트에 있다.** 다이얼로그·툴팁·토스트·상태 필·항목 ID 뱃지. 화면 컴포넌트가 이것들을 다시 만들지 않는다
