...
| 소유자 | — | 개념 아님. 프로젝트의 속성 — 등록한 사용자([[SYNC-PRD-001#R12]]). v2에서 생겼다 |
...
**프로젝트**는 하나의 명세 체인이 사는 단위다. 프로젝트 코드(영문 대문자 4자 이내)로 식별하며 문서 ID의 앞부분이 된다. **등록한 사용자가 소유자다**([[SYNC-PRD-001#R12]]). 소유자는 한 명이고 바뀌지 않으며, 소유자가 아닌 사람에게 그 프로젝트는 없는 것과 같다 — 목록에도 없고 주소로 열어도 없다고 답한다. 역할은 없다. 소유자는 다 하고, 아닌 사람은 보지 못한다.
...
**사용자**는 GitHub 계정으로 식별한다. 인프라 5장에서 GitHub OAuth로 정했으므로 별도 비밀번호나 프로필이 없다. 자기가 등록한 프로젝트를 소유한다. **자리표시 사용자는 소유할 수 없다** — 등록에는 토큰이 필요하고 자리표시에는 토큰이 없다([[SYNC-DOM-002]] 5장 결정 3).
...
**프로젝트가 소유자를 갖지만 사용자 객체를 들지 않는다.** 계정 묶음의 ID(`owner_user_id`)만 갖는다 — 위 규칙 그대로다. 「내 것인가」 판정은 ID 비교 하나라 사용자 객체가 필요 없다.
