...
**빈도만으로 판정하지 않는다.** 빈도가 낮아도 독립적으로 살아야 하는 것이 있고(사용자, 전파 결정), 빈도가 높아도 다른 것의 속성인 것이 있다(커밋, 본문). 아래 셋은 흐름 문장에 명사로 거의 안 나오지만 인프라 설계와 실제 운영에서 필요해진 것이다.
...
> 이것이 없으면 "저장은 됐는데 전파 여부를 아직 안 정한 버전"을 표현할 방법이 없다. [[SYNC-UC-001#UC-H10]] 확장 `2b1`("사람이 결정하지 않고 닫으면 `미결정`이 그대로 남고 내 할 일에 계속 뜬다")도 불가능해진다.
...
개념이 14개라 그대로 두면 읽기 어렵다. 세 가지를 물어 묶는다.
