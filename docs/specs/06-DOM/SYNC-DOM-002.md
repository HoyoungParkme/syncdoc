...
- 폴더는 도메인 묶음 4개 기준. 그 안에 계층
- 기본키는 대리키. 문서 ID·항목 ID는 unique 제약
- 알림 개체 없음. 프로젝트 상세가 문서·참조의 속성(규약 오류·미완성·끊어진 참조)을 직접 센다
...
└── src/
    ...
    ├── components/            두 화면 이상이 쓰는 조각만 — Shell · ui · panes · DiffBox · ItemChain · DiagramFull · HowTo · SettingsDialog
...
서비스·`pipeline`·`queries`가 주고받는 내부 타입. MINISPEC 9개 문서가 전부 여기를 참조한다. API 응답 스키마([[SYNC-API-001]] 4장)와 같은 이름은 그것을 그대로 쓴다 — `Document` `DocumentSummary` `Version` `Diff` `SaveResult` `RebuildResult` `RepoStatus` `ItemRef`. 여기는 API에 없는 것만.
...
| `ItemView` | `doc_id` · `item_id` · `display_name` · `body: str` · `doc_status: DocStatus` · `doc_version_no: int` | get_item → queries |
| `Document` (DTO) | API `Document` 스키마 + `id: int`(행 pk) · `current_version_id: int`(최근 versions.id) · `last_author: AuthorRef` · `missing_refs: list[str]`(미존재 참조 raw_target) | get_document. ORM은 `DocumentRow`(DEV-2) |
...
| `BrokenRefSummary` | `type: "broken_ref"` · `source: ItemRef`(항목 밖 참조면 `item_id=None`) · `raw_target: str` | queries.project_items → API `BrokenRefSummary` |
| `DownstreamView` | `by_item: dict[str, list[ItemRef]]` · `by_document: list[{doc_id, title, items}]` | queries.downstream_view → 추적표 |
| `VersionBrief` | `id` · `document_id` · `version_no` · `commit_hash` · `author: AuthorRef` · `created_at` · `message` | versions_by_ids |
| `ItemRef` (DTO) | API 스키마 + `deleted_at`(내부. API로 안 나감) | describe_items. 삭제된 항목을 가리킬 때 표시용 |
...
**읽는 법** — 시그니처는 API 명세([[SYNC-API-001]], [[SYNC-API-002]])와 시퀀스([[SYNC-SEQ-001]])에서 확정한 것. 반환 타입 중 `Document`·`Diff`·`RepoStatus` 같은 응답 형태는 API 스키마와 같은 이름이다. `-`로 시작하는 메서드는 서비스 안에서만 쓰는 것. 각 그림 아래 표에 메서드마다 어디서 부르는지·유스케이스·던지는 에러를 붙였다. 그림 안 엔티티는 2장의 속성을 그대로 다시 그린다 — 설계 클래스 다이어그램은 서비스와 엔티티 속성이 한 그림에 있어야 읽힌다. **원본에 속성이 두 번 적히는 것이므로 어긋나면 2장이 진실이다.** mermaid 안 클래스는 항목이 아니라(STD-001 1.5) 중복 항목이 되지는 않는다.
...
| `get_item` | queries.item_view | [[SYNC-UC-001#UC-A3]] | not-found(+available_items), item-deleted |
...
| `diff` | queries.diff_with_impact | [[SYNC-UC-001#UC-H6]] | |
...
| `last_author` | queries | [[SYNC-UC-001#UC-H14]] | |
| `recent_changes` | queries | [[SYNC-UC-001#UC-H14]] | |
...
classDiagram
    class ReferenceService {
        ...
        +mark_missing(item_pks: list~int~) int
        +count_missing_by_document(document_ids: list~int~) dict~int,int~
        +missing_in_project(project_id: int) list~RefEdge~
...
| `mark_missing` | pipeline (항목 삭제 확정 · 휴지통) | [[SYNC-UC-001#UC-H13]] 5 |
| `count_missing_by_document` · `missing_in_project` | queries.project_summary · document_list · project_items | [[SYNC-UC-001#UC-H14]] |
...
- `mark_missing`: `to_item_id`가 `item_pks`에 든 참조를 `to_item_id`·`to_document_id` 둘 다 null, `is_missing=true`로 되돌린다 — 체크 제약이 그 둘을 함께 요구한다. `raw_target`은 그대로라 상대가 돌아오면 `resolve_missing`이 다시 잇는다. 끊어진 참조에 별도 개체가 없다([[SYNC-DOM-001]] 3.3)
...
save_pipeline(entry: Entry, doc_id: str | None, doc_type: DocType | None,
    ...
    changed_items mcp: 에이전트가 지정 · web_revert·github: None
    ...
    6. 한 트랜잭션:
         ...
         reference.mark_missing(deleted_item_pks)  (삭제 확정 시)
         reference.extract
         reference.resolve_missing
...
trash_document(doc_id, author, confirm) -> TrashResult
    ...
    → reference.mark_missing(항목 pk). 시퀀스 SEQ-22

restore_document(doc_id, author) -> SaveResult
    되살리기. 휴지통 커밋의 부모에서 파일을 읽어 save_pipeline(entry=web_revert|mcp)로 다시 저장
    → spec.save가 trashed_at을 비우고 항목을 복구 → reference.resolve_missing. 시퀀스 SEQ-23
...
**1. `flags.assignee` — v2에서 사라졌다.** v1의 결정은 「대상 문서 최근 버전의 `author_user_id`」였다. 플래그가 없어지면서 물음 자체가 없어졌다([[SYNC-DOM-001]] 3.3).
