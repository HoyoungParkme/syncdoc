...
**한 화면에 여러 유스케이스가 모인 곳**: UI-5 문서 뷰(H2·H3·H8·H9·H18·H19). 문서를 읽으면서 참조를 보고, 상태를 바꾸고, 댓글을 달고, 막히면 그 자리에서 묻는 건 한 맥락이다. 화면을 나누면 오가느라 흐름이 끊긴다.
...
페이지. 유저용(사람용 뷰, 기본)과 원본(MD)을 탭으로. 참조 보고, 상태 바꾸고, 댓글 달고, 보고 있는 항목에 대해 묻는다. 주 유스케이스: UC-H2·H3·H8·H9·H19
...
페이지. 내 플래그·전파 미결정·규약 오류·댓글. 주 유스케이스: [[SYNC-UC-001#UC-H15]] 프로젝트가 둘 이상이면 프로젝트로 먼저 가르고 칩으로 거른다.
...
오른쪽에 미해결 댓글 수와 `상태 변경 ▾`.

```
├──────────────────────────────────────────────────────┤
│ 싱크독 › 2 PRD › SYNC-PRD-001 [승인] v7   │  미해결 댓글 2  [상태 변경 ▾] │
├──────────────────────────────────────────────────────┤
```
...
**6. 읽는 중 질의를 별도 화면으로 둘 것인가 — 결정: UI-5 오른쪽 패널의 탭.** 별도 화면이면 **무엇에 대해 묻는지를 사람이 다시 지정해야 한다.** 문서 뷰에서는 보고 있는 항목이 이미 선택 상태라 그 지정이 공짜다. 패널은 이미 「이 항목에 딸린 것」이 모이는 자리이기도 하다 — 참조 탭과 댓글 탭이 그렇다. 새 화면을 만들면 요소 번호 한 벌과 진입·이탈 행과 흐름 그림이 더 생기는데 얻는 것이 없다. 탭 하나면 배치에 한 줄이 는다.
