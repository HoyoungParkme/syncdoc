...
<div class="docbar" data-el="1"><!-- 브레드크럼. 어디서 들어왔든 지금 자리를 말한다 -->
  ...
  <span class="btn danger" data-el="12">문서 삭제</span><!-- 초안일 때만. 승인·검토중이면 이 자리가 비어 있다 -->
</div>
...
<div class="dialog narrow" data-el="13">
  <div class="dhead">문서 삭제 — SYNC-PRD-001</div>
  <div class="dbody">
    <p data-el="13.1"><b>제품 요구사항 — 싱크독</b> · 버전 7개 · 파일이 저장소에서 지워지고 버전째 사라집니다. <b>되돌릴 수 없습니다.</b></p>
    <div class="banner warn" data-el="13.2">지울 수 없습니다 — 이력이 있습니다<br>
      · 상태 <b>승인</b> (초안만 지울 수 있다)<br>
      · 들어오는 참조 3 — <b>SYNC-UC-001#UC-A6</b> · <b>SYNC-UC-001#UC-A1</b> · <b>SYNC-API-002</b><br>
      · 댓글 2 · 플래그 1 · 전파 결정 0 · 상태 변경 4</div>
    <div class="dacts"><span class="btn" data-el="13.4">닫기</span> <span class="btn danger" data-el="13.3">삭제</span></div>
  </div>
</div>
```
...
| 12 | 문서 삭제 | 버튼 | 상태 변경(3) 오른쪽. `위험` 색. **초안일 때만 있다** — 검토중·승인 문서는 이력이 있는 문서라 버튼 자체가 없다 | 삭제 확인(13) |
| 13 | 삭제 확인 | 다이얼로그 | 폭 560. 제목·버전 수·되돌릴 수 없음(13.1). 걸리는 것이 있으면 13.2 | — |
| 13.1 | 무엇이 지워지나 | 텍스트 | 문서 제목 · 버전 수 · 「파일이 저장소에서 지워지고 버전째 사라진다. 되돌릴 수 없다」 | — |
| 13.2 | 걸리는 것 | 배너 | `document-has-history`가 실어 온 것 — 상태 · 들어오는 참조(문서ID#항목ID, 각각 링크) · 댓글·플래그·전파 결정·상태 변경 수. 0인 것도 적는다. **없으면 배너 자체가 없다** | 참조 → 그 문서의 UI-5 새 탭 |
| 13.3 | 삭제 | 버튼 | `위험` 색. 13.2가 있으면 비활성 | `DELETE /api/docs/{docId}` → UI-4로. 실패(409)면 13.2를 채워 다시 보여준다 |
| 13.4 | 닫기 | 버튼 | | 아무 일 없음(UC-H18 3b) |
...
- **문서 삭제(12)는 초안에만 있다.** 이력 있는 문서는 지울 수 없고(PRD N3), 검토중·승인은 그 자체가 이력이다. 초안이라도 참조·댓글·플래그가 있으면 확인(13)이 걸리는 것을 보여주고 막는다 — 걸리는 것을 알아야 사람이 그 문서부터 고친다. 화면이 먼저 물어보지 않고 서버 답으로 채운다 — 판정은 한 곳(`pipeline.delete_document`)에서만
