...
| 8.4 | 질문 탭 | 탭 | 보고 있는 항목에 대해 묻는다(UC-H19). **모델 키가 없으면 이 탭이 없다** — `GET /api/me`의 `llm_enabled`로 안다 | 패널을 질문으로 |
...
- **대화는 저장되지 않는다.** 문서를 옮기거나 새로고침하면 사라진다. 남기는 길을 화면에 두지 않는다 — 답을 명세에 옮기는 것은 사람과 그 사람의 에이전트가 한다([[SYNC-PRD-001]] 2장 「전달」의 경계)
...
5. 다른 문서로 옮기면 대화가 사라진다. 남길 값이 있었으면 에이전트에게 말해 명세를 고치게 한다
