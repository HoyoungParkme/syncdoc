...
| 부분 | 무엇 | 형태 |
|---|---|---|
| 배치 | 요소가 어디 있나 | 디자인 도구가 만든 자기 완결 html — 공통 틀 절의 `<link>`·`<style>` + 화면 자신의 `<style>` + 마크업. 아트보드 폭 1280, 높이 800이 기본이고 내용이 길면 자란다. 요소마다 `data-el` 번호. 뷰는 iframe으로 격리해 **본문 폭 전체**에 그대로 그리고, 요소 표·규칙·시나리오는 그 아래에 쌓는다(카드 Z·AA·AC). 한 화면에 상태가 여럿이면(원본 탭·다이얼로그·배너) 주 아트보드 아래에 작은 보드로 잇고 앞에 회색 주석 한 줄을 둔다 |
| 요소 | 각 요소가 뭘 보여주고 누르면 뭐 되나 | 표 |
| 규칙 | 상태별 표시, 조건부 노출, 유효성 | 목록 |
| 시나리오 | 요소들이 이어져 무엇을 이루나 | 번호 매긴 흐름. 유스케이스 흐름을 이 화면 동작으로 옮긴 것 |

사람용 뷰는 배치를 **위에** 본문 폭으로, 요소·규칙·시나리오를 **그 아래에** 렌더링한다(카드 AC). 요소 번호를 누르면 양쪽이 서로 강조된다.
...
<div class="var">화면(UI) 문서를 열었을 때 — 배치가 본문 폭을 다 쓰고, 위에 도구 줄, 아래에 요소 표 (카드 AC)</div>
<div class="sd h" style="min-height:320px">
  <div style="padding:14px 26px;background:#fff">
    <div class="tabs" style="margin-bottom:10px"><span class="tab cur">UI-1 로그인</span><span class="tab">UI-2 프로젝트 목록</span><span class="tab">UI-5 문서 뷰</span></div>
    <div class="wfbar-x" data-el="7.8" style="display:flex;align-items:center;gap:8px;padding:0 0 6px;font-size:11.5px;color:#5C6B73">
      <span class="m">1280×800 · 74%</span><span class="sp"></span>
      <span class="b sm" data-el="7.9">원래 크기</span><span class="b sm" data-el="7.10">전체보기</span>
    </div>
    <div data-el="7.7" style="border:1px solid #C9CFCB;background:#fff;height:150px;display:flex;align-items:center;justify-content:center;color:#8A969C;font-size:12px">배치 — iframe으로 격리된 화면 그림 (본문 폭 전체)</div>
    <div style="margin-top:14px;border:1px solid #C9CFCB;background:#fff;padding:10px 12px;font-size:12px;color:#5C6B73">요소 · 규칙 · 시나리오 — 배치 아래에 쌓인다</div>
  </div>
</div>
```
...
| 2.3 | 이력 | 탭 | | UI-7로 |
| 3 | 상태 토글 | 버튼 | 초안이면 `완료로`, 완료면 `초안으로`. 갈 곳이 하나라 고를 것이 없다 | 전환(UC-H8). **저장소에 아직 안 읽은 커밋이 있으면 먼저 읽고 바꾼다**(UC-H8 1d) — 그만큼 잠깐 걸릴 수 있고, 끝나면 화면이 최신이다. 완료로 올릴 때 규약 오류·미완성·끊어진 참조가 있으면 서버가 `status-blocked`로 거절하고 토스트에 이유 — 배너(4·4a)가 이미 말하는 값이다. 다이얼로그 없음 |
...
| 7.6 | 그림 전체보기 | 층 | 화면 전체. 위 바에 그림 이름 · `－ 100% ＋` · 닫기, 아래에 원본 SVG 복제(공통 1.7). **배치(7.7)도 같은 층에 뜬다** — 그때는 같은 srcdoc의 iframe이다(카드 AC) | 닫기·Esc·바깥 클릭 → 닫힘 |
| 7.7 | 배치 | 그림 | 화면 문서의 배치 html을 iframe으로 격리해 그린 것([[SYNC-STD-002]] V-UI). **본문 폭을 다 쓴다** — 권장 1280은 넓은 창에서 1:1, 좁으면 축소 | 요소를 누르면 아래 요소 표의 그 행이 강조된다. 배치 안 링크는 안 눌린다 |
| 7.8 | 배치 도구 줄 | 영역 | 배치 **위** 한 줄. 왼쪽에 자연폭과 배율(`1280×800 · 74%`), 오른쪽에 7.9·7.10. 배치를 가리지 않는다 | — |
| 7.9 | 맞춤/원래 크기 | 버튼 | 축소된 배치에만. 누르면 1:1과 오간다 | 1:1일 때는 가로 스크롤 |
      ...
      <div class="db">
        ...
        <div class="form" data-el="2"><!-- 입력 넷을 상자 하나로 -->
          ...
          <div class="will" data-el="2.5">
            <b>커밋될 것</b>
            <span class="m">docs/specs/{01-RFQ, 02-PRD, … , 11-CODE}/ · STD/</span>
            <span class="m">docs/specs/assets/</span>
            <span class="m">docs/specs/README.md — 규약 링크</span>
          </div>
        </div>
      </div>
...
| 2.5 | 커밋될 것 | 영역 | 등록하면 저장소에 무엇이 생기는지 — 빈 단계 디렉터리와 README 하나다. **규약·템플릿 사본은 안 넣는다**(카드 AB). 빈 저장소가 아니면 안 보인다 | — |
...
| 5.1 | 집계 | 텍스트 | 읽은 문서·항목·참조·버전 수. README를 새 판으로 커밋했으면 「README 갱신됨」이 함께(`readme_updated`) | — |
...
- **재구축이 저장소에 쓰는 것은 `docs/specs/README.md` 하나뿐이다.** 인덱스를 다시 만들기 전에 README가 지금 판과 다르면 새 판으로 커밋한다 — 사본은 규약이 바뀌어도 저절로 갱신되지 않기 때문이다(카드 AB, #129). 같으면 커밋이 없다. 명세 MD는 읽기만 한다
...
- **UI는 「문서 1~2」로 적는다.** 규약이 한 문서도 둘도 허용한다([[SYNC-STD-001]] 2.7) — 앱 안내가 「둘」로 못박으면 규약보다 좁은 말을 가르친다(카드 AE)
