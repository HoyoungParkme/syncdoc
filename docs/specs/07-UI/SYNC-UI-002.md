...
| 유스케이스 | [[SYNC-UC-001#UC-H2]] [[SYNC-UC-001#UC-H3]] [[SYNC-UC-001#UC-H8]] [[SYNC-UC-001#UC-H9]] [[SYNC-UC-001#UC-H19]] |
...
<div class="body3" data-el="7">
  ...
  <aside class="panel" data-el="8">
    <div class="ptabs"><span class="on" data-el="8.1">참조</span><span data-el="8.2">댓글 2</span><span data-el="8.4">질문</span></div>
    <div class="pbody">
...
<div class="dialog narrow" data-el="13">
  ...
  <div class="dbody">
    ...
    <div class="banner warn" data-el="13.2">넣으면 끊어지는 것<br>
      · 들어오는 참조 3 — <b>SYNC-UC-001#UC-A6</b> · <b>SYNC-UC-001#UC-A1</b> · <b>SYNC-API-002</b> → 그 항목에 <b>끊어진 참조</b>가 붙습니다<br>
      · 댓글 2 — 휴지통에 같이 있다가 되살리면 돌아옵니다</div>
...
<aside class="panel">
  <div class="ptabs"><span data-el="8.1">참조</span><span data-el="8.2">댓글 2</span><span class="on">질문</span></div>
  <div class="pbody">
    <div class="lbl" data-el="8.5">R1 에이전트용 원본과 사람용 뷰 · 이 항목에 대해 묻습니다</div>
    <div class="qa" data-el="8.7">
      ...
      <div class="qsrc">본 것: R1 · SYNC-RFQ-001#Q1 · SYNC-UC-001#UC-H2</div>
    </div>
    <div class="qact"><span class="btn" data-el="8.8">댓글로 남기기</span></div>
    <textarea data-el="8.6" placeholder="이 항목에 대해 묻습니다"></textarea>
  </div>
</aside>

<!-- 휴지통에 있는 문서를 열면 (4b). 상태 변경(3)·삭제(12)·댓글 버튼은 없다 -->
...
| 4b | 휴지통 배너 | 배너 | `trashed_at`이 있으면. 언제·누가 넣었나, 되살리면 돌아온다는 것. 이 배너가 있으면 3·5·7.4·12가 없다 | — |
| 4b.1 | 되살리기 | 버튼 | 배너 안 | `POST /api/docs/{docId}/restore` → 같은 문서 새 버전(UC-H18 5) |
| 4a | 미완성 배너 | 배너 | 미완성 경고(`incomplete_warnings`) **+ 가리키는 곳이 없는 참조**(`missing_refs`). 둘을 이어 붙여 한 배너로. **[[SYNC-STD-001]] 4장 `화면 문구` 열을 쓴다** — `warnings`가 실어 오는 `rule`은 기계가 읽는 ID다. 규약 오류와 색이 다르고, 저장은 됐고 승인만 막힘 | — |
...
| 7.4 | 댓글 버튼 | 버튼 | 본문 **문단** 오른쪽. `+` 또는 그 줄의 댓글 개수(있으면 주의색 테두리) | 패널(8) 댓글 탭을 열고 그 줄을 잡는다 |
...
| 8.4 | 질문 탭 | 탭 | 보고 있는 항목에 대해 묻는다(UC-H19). **모델 키가 없으면 이 탭이 없다** | 패널을 질문으로 |
| 8.5 | 맥락 줄 | 텍스트 | 지금 무엇에 대해 묻는지 — 선택 항목의 ID와 제목. 항목을 안 골랐으면 「항목을 선택하세요」 | — |
| 8.6 | 질문 입력 | 입력 | 보내면 8.7에 쌓인다. 답을 기다리는 동안 기다린다고 말한다 | — |
| 8.7 | 대화 | 목록 | 질문과 답이 차례로. **저장되지 않는다** — 문서를 옮기거나 새로고침하면 사라진다. 답 아래 「본 것」에 맥락으로 실은 항목 ID | 항목 ID → 7.2와 같음 |
| 8.8 | 댓글로 남기기 | 버튼 | 답 하나를 항목 헤딩 줄의 댓글로(UC-H19 4b) | 댓글 탭(8.2)으로 바뀌고 5번이 1 늚 |
...
| 12 | 휴지통에 넣기 | 버튼 | 상태 변경(3) 오른쪽. `위험` 색. 휴지통에 있는 문서면 없다 | 확인(13) |
| 13 | 휴지통 확인 | 다이얼로그 | 폭 560. 제목·버전 수·되살릴 수 있음(13.1). 끊어질 것(13.2) | — |
| 13.1 | 무엇이 되나 | 텍스트 | 문서 제목 · 버전 수 · 「파일이 저장소에서 지워지고 행과 이력은 남아 되살릴 수 있다」 | — |
| 13.2 | 끊어지는 것 | 배너 | `document-deletion-needs-confirm`의 `inbound_refs`(문서ID#항목ID, 각각 링크)와 `comments`. **막지 않는다** — 알고 넣게 한다. 0이면 배너 없음 | 참조 → 그 문서의 UI-5 새 탭 |
...
- **댓글 버튼(7.4)은 문단에만 붙는다.** 제목·표·목록에는 없다 — 댓글은 줄에 달리는데 표와 목록은 한 줄이 여러 칸으로 흩어져 어느 줄인지 가리킬 자리가 없다. 그런 줄에 달린 댓글은 패널(8.2) 목록과 줄 입력으로 다룬다
- 버튼이 잡는 줄 번호는 **원본 MD 기준**이다. 뷰가 문단마다 그 문단 첫 줄의 원본 텍스트를 실어 주고 화면이 그걸로 줄을 찾는다 — 타입별 렌더러가 본문을 절·항목으로 여러 번 쪼개므로 블록 안 위치로는 원본 줄을 알 수 없다([[SYNC-STD-002]] 6장)
- 처음 열면 패널(8)은 참조 탭이고 "항목을 선택하세요"
- 규약 오류(4)가 있으면 3(상태 변경)이 비활성이고 배너에 이유 표시. 미완성(4a)이면 3에서 `승인`만 비활성. 배너는 유저용·원본 양쪽에 보인다
- **끊어진 참조도 미완성(4a)이다.** 저장은 됐고 승인만 막힌다. 가리키는 문서가 아직 안 쓰인 것일 수 있고, 그 문서가 들어오면 저절로 풀린다 — 이 문서를 다시 저장하지 않아도 된다. 그래서 배너가 보여주는 값과 승인을 막는 값이 **같은 곳에서 온다**(`missing_refs`)
- `승인`은 상위 대조(11)를 건너뛸 수 없다. 상위 항목이 하나도 없는 문서(RFQ)는 다이얼로그가 "상위 없음"으로 뜨고 바로 승인
- **지우기는 휴지통이다(PRD N3).** 어떤 문서든 넣을 수 있고, 확인(13)은 막는 게 아니라 무엇이 끊어지는지 보여준다 — 13.2는 화면이 세지 않고 서버가 `document-deletion-needs-confirm`으로 준 값이다(웹은 먼저 `confirm` 없이 불러 그 목록을 받고, 넣기를 누르면 `confirm=true`로 다시 부른다. 판정은 한 곳 `pipeline.trash_document`). 휴지통에 있는 문서는 4b 배너 하나로 말하고 상태 변경·댓글·넣기 버튼이 없다. 되살리기(4b.1)와 완전 삭제는 UI-4 휴지통 묶음(8)에도 있다
- 참조 링크(7.2)가 미존재 참조면 회색 + 경고 아이콘, 클릭해도 이동 없음
- **질문 탭(8.4)은 기본 탭이 아니다.** 처음 열면 여전히 참조 탭이고, 7.1은 참조 탭으로 7.4는 댓글 탭으로 가는 기존 전환도 그대로다. 질문 탭은 사람이 직접 누를 때만 열린다. URL은 `?panel=ask`
- **항목을 고르지 않으면 못 묻는다.** 8.5가 「항목을 선택하세요」이고 입력(8.6)이 비활성이다 — 무엇에 대한 질문인지가 곧 맥락이라 문서 전체에 대한 질문은 받지 않는다
- **모델 키가 없으면 탭 자체가 없다**(인프라 5.3). 참조·댓글 두 탭만 보이고 나머지 화면은 그대로다. 꺼진 기능을 회색으로 남겨 두지 않는다 — 켤 방법이 사람에게 없다
- **대화는 저장되지 않는다.** 문서를 옮기거나 새로고침하면 사라진다. 남기려면 8.8로 댓글이 되고, 그때 **작성자는 누른 사람이다** — 모델은 계정이 없다
- 답이 길어도 **패널 안에서만 스크롤한다**. 3단 규칙 그대로
- **휴지통에 있는 문서(4b)에는 질문 탭이 없다.** 파일이 저장소에서 지워진 상태라 물을 본문이 없다 — 3·5·7.4가 없는 것과 같은 이유다
...
1. 항목 헤더(7.1)를 클릭한다. 패널(8)이 참조 탭으로 바뀐다
2. 질문 탭(8.4)을 누른다. 맥락 줄(8.5)에 그 항목의 ID와 제목이 떠 있다
3. 입력(8.6)에 묻는다. 답(8.7)이 뜨고 아래 「본 것」에 맥락으로 실은 항목 ID가 보인다
4. 이어 물으면 앞 대화가 함께 간다
5. 남길 값이 있어 「댓글로 남기기」(8.8)를 누른다. 패널이 댓글 탭으로 바뀌고 5번이 1 늘어난다
6. 다른 문서로 옮기면 대화가 사라진다. 남긴 댓글만 남아 있다
...
<div class="dialog narrow" data-el="8.4">
  ...
  <div class="dbody">행까지 지워지고 <b>되돌릴 수 없습니다.</b> 번호는 다시 쓰일 수 있습니다.
    <div class="banner warn">지울 수 없습니다 — 아직 가리키는 곳이 있습니다<br>· <b>SYNC-DOM-002#Document</b> · 댓글 1</div>
...
<!-- 프로젝트가 둘 이상일 때만 — 칩(10)으로 거르고, 묶음(11)으로 가른다. 하나면 이 둘 없이 아래 종류 묶음만 -->
<div class="tchips" data-el="10">
  <span class="btn sm on" data-el="10.1">전체 <span class="cnt">12</span></span>
  <span class="btn sm"><b class="mono">[SYNC]</b> 싱크독 <span class="cnt">7</span></span>
  <span class="btn sm"><b class="mono">[INS]</b> 보험청구심사 어시스턴트 <span class="cnt">5</span></span>
</div>
<div class="todo">

  <!-- 카드 경계는 묶음이 아니라 항목에 있다. 묶음 머리는 배경 위 맨 텍스트다 -->
  <section class="tproj" data-el="11"><!-- 프로젝트 묶음. 순서는 그 안 가장 오래된 일 순 -->
  <h3 class="tph" data-el="11.1"><b class="mono">[SYNC]</b> 싱크독 <span class="cnt">7</span><span class="rule"></span></h3>
  ...
  </section><!-- /tproj [SYNC] — 다음 프로젝트 묶음 [INS]가 같은 모양으로 이어진다 -->
...
| 10 | 프로젝트 칩 | 영역 | 머리 아래 한 줄. `전체 n` 뒤에 프로젝트마다 `[코드] 이름 n`(1.6). **프로젝트가 둘 이상일 때만** | — |
| 10.1 | 칩 | 버튼 | 하나만 켜진다. 처음은 전체 | 그 프로젝트 묶음(11)만 남는다. 순서·건수(1.1)는 안 바뀐다 |
| 11 | 프로젝트 묶음 | 묶음 | 한 프로젝트의 일 전부 — 안에 종류 묶음(2~7)이 그대로. **프로젝트가 둘 이상일 때만** | — |
| 11.1 | 프로젝트 머리 | 텍스트 | `[코드] 이름 · 건수`와 열 끝까지 가는 실선. 종류 묶음 머리보다 한 단계 크다 | — |
...
- **프로젝트가 둘 이상이면 프로젝트로 먼저 가른다(11), 그 안이 종류 묶음이다.** 하나면 칩(10)도 묶음 머리(11.1)도 안 그린다 — 2~3명 프로젝트 하나짜리 화면이 지금과 같아야 한다. 넷이 섞인 목록에서 `SYNC-PRD-001#R1`의 접두만으로는 훑어서 안 갈렸다
- **프로젝트 묶음의 순서는 그 안 가장 오래된 일 순이다.** 경과일순이 묶음 사이에도 성립한다 — 가장 방치된 프로젝트가 위. 묶음 안은 종류 순서 고정·경과일순 그대로
- **칩(10)은 필터일 뿐이다.** 순서를 바꾸지 않고, 부제(1.1)의 건수와 상단 바 배지도 그대로다 — 거른 건 화면이지 할 일이 아니다
- 담당 미지정(7)도 그 프로젝트 묶음 안 마지막이다. 프로젝트 밖으로 빼면 「누구 것도 아닌 것」이 「어느 프로젝트 것도 아닌 것」으로 읽힌다
...
**S-5 프로젝트 넷이 섞였다**
1. 칩(10)에 `전체 12 · [SYNC] 7 · [INS] 5`. 목록은 [SYNC] 묶음(11)이 위 — 가장 오래된 일이 거기 있다
2. `[INS]` 칩을 누른다. [INS] 묶음만 남고 부제는 여전히 `12건`
3. 카드를 눌러 처리하고 돌아오면 그 칩이 켜진 채다
...
| 2.2 | 원인 버전 범위 | 텍스트 | **플래그를 만든 변경의 직전 버전 → 현재 버전.** 부여 시점 버전을 시작점으로 쓰면 부여 직후에 `v2 → v2`가 되어 빈다. 부여 뒤 또 바뀌었으면 횟수도(UC-H11 3a) | — |
...
- **커밋 이메일(2.3~2.6)은 GitHub 계정과 커밋 작성자를 잇는 다리다.** git 커밋이 남기는 신원은 이름과 이메일뿐이고 이름은 아무 문자열이라 계정과 못 잇는다. 등록 안 하면 GitHub에서 바로 push한 내 커밋이 **다른 사람으로 잡혀 내 할 일(UI-10)이 빈다**
- **등록만으로는 이미 쌓인 것이 안 옮겨진다.** 버전·플래그는 인덱스를 다시 만들 때 작성자를 다시 찾는다. **등록 → 관리(5)에서 인덱스 재구축** 순서로 해야 한다. 이 안내를 카드에 둔다
...
<div class="adminbody">
  ...
  <div class="dialog" data-el="4">
    ...
    <div class="dbody">
      ...
      <div class="banner warn" data-el="4.3"><b>GitHub 저장소는 그대로 남습니다</b> — 다시 등록하면 문서가 git에서 복원됩니다. 플래그·전파 결정·댓글은 <b>backup/tracking.json</b>이 있을 때만 돌아옵니다 — 이 프로젝트는 백업이 없습니다</div>
  ...
  <section class="grp" data-el="5">
    ...
    <div class="row"><span class="warn" data-el="5.3">버린 것 2 — 전파 결정 2건 · 가리키던 커밋이 저장소에 없습니다</span></div>
  </section>
</div>
```
...
| 4.2 | 취소 | 버튼 | | 닫힘 |
| 4.3 | 해제 안내 | 텍스트 | 해제(7)일 때만. 저장소는 남는다 · 재등록하면 문서가 복원된다 · 플래그·전파 결정·댓글은 백업(2.4)이 있을 때만 돌아온다 — **그 행의 백업 유무를 그대로 말한다** | — |
...
| 7 | 해제 | 버튼 | 행마다, 복원(6) 옆. 위험색 외곽선 — 토큰 폐기(UI-13 3.2)와 같은 어휘 | 확인 다이얼로그(4, 안내 4.3) → [[SYNC-API-001#DELETE/api/projects/{code}]]. 끝나면 UI-2로 |
...
- **「그대로다」에 예외가 하나 있고, 그 예외는 조용하지 않다.** 플래그·전파결정은 버전을 가리킨다. 재구축이 버전을 다시 만들 때 커밋 해시로 다시 잇는데([[SYNC-MS-004#TrackingService.relink_versions]]), **가리키던 커밋이 저장소에 없으면 이을 데가 없다** — force-push로 사라졌거나, 문서가 삭제됐거나, `status(` 커밋이라 버전이 안 생기는 경우다. 그때만 그 행을 버리고 **몇 건을 왜 버렸는지 5.3에 적는다.** 확인 문구(4)가 "건드리지 않습니다"라고 약속하므로, 예외가 생겼으면 말해야 그 약속이 거짓이 안 된다
- **버전 번호는 바뀔 수 있다.** 재구축은 커밋마다 버전을 만들므로 번호가 전부 다시 매겨진다. 커밋 해시로 잇기 때문에 **가리키는 대상은 그대로이고 표시되는 `v번호`만 움직인다** — UI-10·UI-12의 버전 표시가 재구축 전후로 다를 수 있다
...
- **해제(7)는 싱크독의 등록·색인·작업 사본만 지운다.** GitHub 저장소는 손대지 않는다 — 명세 원본은 거기 있고, 다시 등록하면 문서가 git에서 복원된다(UC-A1 3a2). 그래서 재구축·복원과 같은 표에 둔다: 셋 다 「이 저장소를 싱크독이 어떻게 들고 있나」를 만지는 일이다
- **해제 확인(4)은 되돌릴 수 있는 것과 없는 것을 갈라 말한다(4.3).** 문서는 돌아오고, 플래그·전파 결정·댓글은 백업(2.4)이 있을 때만 돌아온다 — 그 행의 백업 유무를 그대로 적는다. 뭉뚱그려 「정말 지울까요?」라고만 물으면 사람이 무엇을 잃는지 모른 채 누른다
- **해제가 끝나면 UI-2로 간다.** 이 다이얼로그는 어느 화면 위에서든 뜨므로 지운 프로젝트의 상세 위에서 눌렀을 수 있다
- **버튼은 「해제」 두 글자다.** 열을 늘리지 않는다는 위 규칙과 같은 이유 — 셋이 한 칸에 가로로 서야 한다
...
2. 확인(4)에 저장소는 남는다는 것과 이 프로젝트의 백업 유무(4.3)가 뜬다. 해제(4.1)
