...
**복구 불가 항목이 셋 있다.** 원본에 없는, 시스템이 만든 정보이기 때문이다. C2가 보장하는 것은 "명세와 참조가 살아남는다"이지 "모든 것이 살아남는다"가 아니다. **그래서 DB 덤프를 저장소에 함께 커밋한다.** 명세와 무관한 파일이 섞이는 값을 치르고 셋을 살린다. 파이프라인은 `docs/specs/` 아래만 보므로(MS-009 `changed_files`) 덤프 커밋은 변경 감지에 안 걸린다.
...
- [x] 플래그·댓글의 백업 방식 — 6장 참고. DB 덤프를 저장소에 커밋할지, 유실을 감수할지 — 결정: 덤프를 저장소에 커밋한다. 6장 표의 복구 불가 셋을 살린다. 파이프라인은 `docs/specs/` 밖을 보지 않으므로(MS-009 `changed_files`의 prefix) 덤프 커밋이 폴링을 건드리지 않는다
