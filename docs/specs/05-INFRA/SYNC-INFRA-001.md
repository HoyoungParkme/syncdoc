...
| 항목 | 결정 |
|---|---|
| 키 | **서버에 하나**(`LLM_API_KEY`). 사람마다 넣지 않는다 — 실사용이 2~3명이고([[#C6]]), 사람마다 키를 두면 `users`에 컬럼이 늘고 5.1 재암호화가 하나 더 생긴다 |
...
| 테이블 | 싣는 것 | 빼는 것 |
|---|---|---|
| `flags` | 종류·대상 항목·원인 항목·원인 버전·**대상 버전**·담당자·부여 시각·해제자·해제 시각·수정 동반 여부 | — |
