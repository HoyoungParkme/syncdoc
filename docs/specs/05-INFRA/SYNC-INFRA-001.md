...
| 항목 | 결정 |
|---|---|
| 무엇을 | `flags` · `propagation_decisions` · `comments` 세 테이블 |
| 어떤 형식 | **자연키 JSON.** 숫자 `id`가 아니라 `문서ID#항목ID` · 버전 번호 · `github_login`으로 적는다 |
| 어디에 | 저장소 `backup/tracking.json` |
| 언제 | 앱이 하루 한 번. 지난 백업과 내용이 같으면 커밋하지 않는다 |
| 복원 | clone → 인덱스 재구축(UC-S6) → 가져오기. 이름으로 다시 붙는다 |
...
**변경 감지에는 안 걸린다.** 파이프라인은 `docs/specs/` 아래만 본다(MS-009 `changed_files`의 prefix). `backup/`은 폴링이 명세 변경으로 오해하지 않는다.
