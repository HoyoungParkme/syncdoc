...
1. `docs/specs/STD/SYNC-STD-004.md` — 개발 규약. 코드가 지켜야 할 규칙 15개(DEV-1~15)
2. `docs/specs/STD/SYNC-STD-001.md` — 명세 작성 규약. 명세를 고쳐야 할 때
3. `docs/specs/CODE/SYNC-CODE-001.md` — 구현 계획. **여기서 카드 하나를 받는다**
...
2. 카드의 참조를 전부 연다 — 시나리오·유스케이스(왜) → MINISPEC(어떻게) → API·화면(입구)
   참조 형식: [[SYNC-MS-002#SpecService.save]] → docs/specs/MS/SYNC-MS-002.md 의 #### SpecService.save 블록
...
```
docs/specs/{TYPE}/{doc_id}.md     원본 (에이전트가 읽는 것)
