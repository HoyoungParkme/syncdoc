...
- `python tools/view_build.py --selftest` — V-PRD·V-INFRA·V-SCN·V-UI가 원본 문장(항목 본문·절 머리·소절 머리·시나리오 단계와 변형의 둘째 줄부터·설계 화면 블록 전부·화면 규칙과 시나리오의 이어진 줄·「그 밖」)을 버리지 않고 절을 두 번 그리지 않는지. 뷰 코드를 고치면 돌린다
